...
def json_to_newick(json_obj):
	...
	def _parse_json(json_obj):
		...
		if 'levelDistance' in data:
			normDist = (data['levelDistance'])/1.88
			newick = newick + ':' + str(round(normDist,0))
...
def json_to_table(json_obj):
	...
	def _parse_json1(json_obj):
		...
		try:
			...
			newick = ''
		if 'did' in json_obj:
			newick = newick + '\t' + str(json_obj['did'])

		if 'name' in json_obj:
			newick = newick + '\t' + str(json_obj['name'])
		...
		if 'totalDistance' in data:
			normDist = (data['totalDistance'])/1.88
			newick = newick + '\t' + str(round(normDist,0))

		if 'deathDistance' in data:
			normDist = (data['deathDistance'])/1.88
			newick = newick + '\t' + str(round(normDist,0))

		if 'type' in data:
			newick = newick + '\t' + str(data['type'])

		# If there are 'children'	
		if 'children' in json_obj and len(json_obj['children'])>0:
			newick = newick + '\t' + ','.join(child['id'] for child in json_obj['children'])
			normDist = (json_obj['children'][1]['data']['totalDistance'])/1.88
			newick = newick + '\t' + str(round(normDist,0))
			# Initialise a list to contain the daughter info
			info = []
			# For each child, treat it as a new dictionary object
			for child in json_obj['children']:
				# parse the newick string straight into it the list
				info.append(_parse_json1(child))
